...
beauce_ne <- read.table(paste0(ne_dir, "beauce_cluster_hap-ibd_ne.ne"), header = TRUE)
beauce_ne$probands <- "Beauce"
urbanqc_ne <- read.table(paste0(ne_dir, "urbanqc_cluster_hap-ibd_ne.ne"), header = TRUE)
urbanqc_ne$probands <- "UrbanQc"
slsj_ne <- read.table(paste0(ne_dir, "slsj_cluster_hap-ibd_ne.ne"), header = TRUE)
slsj_ne$probands <- "SLSJ"
df1 <- rbind(beauce_ne, urbanqc_ne, slsj_ne)
df2 <- subset(df1, GEN <= 14)

#### ggforce adjustement to plot zoomed portion on the right (functions from user24248721 (https://stackoverflow.com/questions/52665619/how-to-change-the-position-of-the-zoomed-area-from-facet-zoom))
...
region_colors <- c("Beauce" = "red", "UrbanQc" = "orange", "SLSJ" = "blue")
gg1_zoom <- ggplot(df2, aes(x = as.factor(GEN), y = NE, color = probands, group = probands)) +
  geom_line(show.legend = TRUE, linewidth = 1) +
  geom_errorbar(aes(ymin = as.numeric(LWR.95.CI), ymax = as.numeric(UPR.95.CI)), width = 0.2, show.legend = FALSE) +
  labs(x = "Generation", y = "Effective population size", color = "Probands' marriage region") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_color_manual(values = region_colors) +
  theme_light() +
  theme_custom + 
  facet_zoom2(y = probands == c("Beauce", "SLSJ"), zoom.size = 0.5)
gg1_zoom
